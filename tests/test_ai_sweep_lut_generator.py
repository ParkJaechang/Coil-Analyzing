...
from field_analysis.ai_sweep.manifest_io import validate_manifest_dataframe
from field_analysis.ai_sweep.schema import SweepSegmentSpec, SweepTargetConfig
from field_analysis.ai_sweep.sweep_lut_generator import (
    SegmentCommandInput,
...
def test_build_sweep_lut_rejects_voltage_limit_exceedance() -> None:
    with pytest.raises(ValueError, match="voltage_limit_exceeded"):
        build_sweep_lut_from_segment_commands(
            [_segment(values=[0.0, COMMAND_VOLTAGE_LIMIT_V + 0.001, 0.0])]
        )
...
def test_build_sweep_lut_uses_existing_voltage_policy_constant() -> None:
    import field_analysis.ai_sweep.sweep_lut_generator as generator

    source = inspect.getsource(generator)

    assert "COMMAND_VOLTAGE_LIMIT_V" in source
