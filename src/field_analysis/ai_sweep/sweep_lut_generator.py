...
def build_sweep_lut_from_segment_commands(
    ...
    metadata = {
        ...
        "voltage_limit_policy_source": "field_analysis.voltage_policy.COMMAND_VOLTAGE_LIMIT_V",
        "max_abs_voltage_v": float(max_abs_voltage_v),
        "lut_columns": list(lut.columns),
        "final_hardware_lut_columns": list(HARDWARE_LUT_COLUMNS),
        "manifest_columns": list(manifest.columns),
        "ai_sweep_lut_generation_mode": "concatenate_prebuilt_segment_commands",
        "ai_sweep_route_classification": "experimental_offline_planning_only",
        "modeling_core_called": False,
        "production_quick_lut_route_mutated": False,
        "production_lut_export_route": False,
        "user_approval_required_before_runtime_or_hardware": True,
        _UI_FLAG_KEY: False,
...
def _validated_voltage_samples(command_profile: pd.DataFrame, voltage_column: str) -> np.ndarray:
    ...
    if float(np.max(np.abs(voltage))) > float(COMMAND_VOLTAGE_LIMIT_V):
