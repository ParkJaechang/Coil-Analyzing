...
def render_final_voltage_lut_export_panel(
    *,
    ...
    st.caption("저장 컬럼: sample_index, time_s, voltage_v")
    st.caption("voltage_v는 선택한 모델링 결과의 최종 전압 샘플입니다.")
    st.caption("exported CSV uses plotted final command voltage samples; not Fourier, not harmonic resynthesis; columns: sample_index, time_s, voltage_v")
    if not finite_cycle_mode:
        st.info("최종 전압 LUT 추출 사용 불가: finite compensation 결과에서만 다운로드할 수 있습니다.")
        return
    ...
    if missing:
        st.warning(f"최종 전압 LUT 추출 사용 불가: missing columns {missing}")
        return
