...
def synthesize_current_waveform_compensation(
    ...
    selected_support_waveform_family = finite_empirical_model.get("selected_support_waveform_family") if use_finite_empirical_route else None
    selected_support_source_file = finite_empirical_model.get("selected_support_source_file") if use_finite_empirical_route else None
    selected_support_freq_hz = finite_empirical_model.get("selected_support_freq_hz") if use_finite_empirical_route else support_freq_hz
    ...
    support_cycle_override_reason = finite_empirical_model.get("support_cycle_override_reason") if use_finite_empirical_route else None
    ...
    if field_only_route and "target_aligned_support_reference_mT" in command_profile.columns:
        command_profile["support_reference_output_mT"] = command_profile["target_aligned_support_reference_mT"]
    elif field_only_route and "support_scaled_field_mT" in command_profile.columns:
        command_profile["support_reference_output_mT"] = command_profile["support_scaled_field_mT"]
    if field_only_route and "support_reference_output_mT" in command_profile.columns:
        command_profile["target_aligned_support_reference_mT"] = command_profile["support_reference_output_mT"]
    ...
    if field_only_route and finite_cycle_mode:
        ...
        command_profile["support_family_score_summary"] = support_family_score_summary
    ...
    return {
        ...
        "source_total_duration_s": _first_numeric(command_profile.get("source_total_duration_s")),
        "source_pre_baseline_s": _first_numeric(command_profile.get("source_pre_baseline_s")),
        "source_post_tail_s": _first_numeric(command_profile.get("source_post_tail_s")),
        "source_command_active_start_s": _first_numeric(command_profile.get("source_command_active_start_s")),
        "source_command_active_end_s": _first_numeric(command_profile.get("source_command_active_end_s")),
        "source_field_active_start_s": _first_numeric(command_profile.get("source_field_active_start_s")),
        ...
        "command_generation_target": command_prediction_consistency.get("command_generation_target"),
        "support_reference_role": command_prediction_consistency.get("support_reference_role"),
        "support_reference_used_for_command": command_prediction_consistency.get("support_reference_used_for_command"),
        "forward_prediction_source": command_prediction_consistency.get("forward_prediction_source"),
        "predicted_from_plotted_command": command_prediction_consistency.get("predicted_from_plotted_command"),
        "command_prediction_consistency_status": command_prediction_consistency.get("command_prediction_consistency_status"),
        "command_nonzero_start_s": command_prediction_consistency.get("command_nonzero_start_s"),
        "target_nonzero_start_s": command_prediction_consistency.get("target_nonzero_start_s"),
        "command_target_start_delta_s": command_prediction_consistency.get("command_target_start_delta_s"),
        "command_covers_target_active_start": command_prediction_consistency.get("command_covers_target_active_start"),
        "command_covers_target_active_end": command_prediction_consistency.get("command_covers_target_active_end"),
        ...
        "support_family_score_summary": support_family_score_summary,
        "support_cycle_match_type": support_cycle_match_type,
        "support_cycle_override_applied": bool(support_cycle_override_applied),
        "support_cycle_override_reason": support_cycle_override_reason,
...
def _finite_source_signal_window(time_values: np.ndarray, signal_values: np.ndarray) -> tuple[float, float, str]:
    start_s, end_s = _nonzero_window(np.asarray(time_values, dtype=float), np.asarray(signal_values, dtype=float))
    if np.isfinite(start_s) and np.isfinite(end_s) and end_s > start_s:
        return float(start_s), float(end_s), "ok"
    return float("nan"), float("nan"), "active_window_missing"
...
def _resample_finite_support_record(
    *,
    ...
    full_time = pd.to_numeric(full_frame["time_s"], errors="coerce").to_numpy(dtype=float)
    source_total_duration_s = (
        float(np.nanmax(full_time) - np.nanmin(full_time))
        if np.isfinite(full_time).sum() >= 2
        else float("nan")
    )
    command_start_s, command_end_s, command_window_status = _finite_source_signal_window(
        full_time,
        pd.to_numeric(full_frame["daq_input_v"], errors="coerce").to_numpy(dtype=float),
    )
    field_column_for_window = _resolve_finite_signal_column(
        full_frame,
        [str(entry.get("resolved_field_channel") or ""), field_channel, "bz_mT", "bproj_mT", "bmag_mT"],
        field_channel,
    )
    field_start_s, field_end_s, field_window_status = _finite_source_signal_window(
        full_time,
        pd.to_numeric(full_frame[field_column_for_window], errors="coerce").to_numpy(dtype=float)
        if field_column_for_window in full_frame.columns
        else np.zeros(len(full_frame), dtype=float),
    )
    alignment_window = "command_active_window"
    alignment_status = "ok"
    mapping_mode = "active_window_to_target_window"
    ...
    if active_source != "explicit_active_frame" and np.isfinite(command_start_s) and np.isfinite(command_end_s):
        ...
        alignment_window = "unavailable"
        alignment_status = command_window_status if command_window_status != "ok" else field_window_status
        mapping_mode = "raw_source_debug_only"
    if active_frame.empty:
        return None
    ...
    if active_source == "derived_from_cycle_metadata" and np.isfinite(expected_support_active_duration_s):
        if not np.isfinite(command_start_s) and not np.isfinite(field_start_s):
            ...
            if len(derived_active) >= 2:
                ...
                mapping_mode = "full_record_to_target_window"
    ...
    support_reference = np.full_like(time_grid, np.nan, dtype=float)
    voltage[active_mask] = voltage_active
    current[active_mask] = current_active
    field[active_mask] = field_active
    support_reference[active_mask] = field_active
    if tail_mask.any():
    ...
    return {
        ...
        "support_reference_field_mT": support_reference * float(scale_ratio),
        "active_window_start_s": active_start_s,
        "active_window_end_s": active_end_s,
        "active_duration_s": float(active_duration_s),
        "source_total_duration_s": source_total_duration_s,
        "source_pre_baseline_s": source_pre_baseline_s,
        "source_post_tail_s": source_post_tail_s,
        "source_command_active_start_s": command_start_s,
        "source_command_active_end_s": command_end_s,
...
def _build_finite_modeled_profile(
    *,
    ...
    modeled["target_aligned_support_reference_mT"] = np.asarray(
        support_payload.get("support_reference_field_mT", support_payload["field_mT"]),
        dtype=float,
    )
    ...
    if target_output_type == "current":
        ...
        modeled["support_reference_output_mT"] = modeled["target_aligned_support_reference_mT"]
    ...
    for key in (
        "source_total_duration_s",
        "source_pre_baseline_s",
        "source_post_tail_s",
        "source_command_active_start_s",
        "source_command_active_end_s",
        ...
        modeled[key] = support_payload.get(key)
    modeled["support_reference_alignment_status"] = support_payload.get("support_reference_alignment_status", "ok")
    modeled["support_reference_timebase"] = "target_aligned"
    modeled["support_reference_plotted_column"] = "support_reference_output_mT"
    modeled["support_reference_plotted_source"] = "target_aligned_support_reference"
    modeled["support_reference_time_s"] = time_grid
...
def synthesize_finite_empirical_compensation(
    ...
    blended_reference_sum = np.zeros_like(time_grid, dtype=float)
    blended_reference_weight = np.zeros_like(time_grid, dtype=float)
    selected_active_window_start_s = float("nan")
    selected_active_window_end_s = float("nan")
    selected_source_total_duration_s = float("nan")
    selected_source_pre_baseline_s = float("nan")
    selected_source_post_tail_s = float("nan")
    selected_source_command_active_start_s = float("nan")
    selected_source_command_active_end_s = float("nan")
    ...
    for weight, record in zip(normalized_weights, selected_records, strict=False):
        ...
        if not np.isfinite(selected_source_total_duration_s):
            selected_source_total_duration_s = float(support_payload.get("source_total_duration_s", np.nan))
            selected_source_pre_baseline_s = float(support_payload.get("source_pre_baseline_s", np.nan))
            selected_source_post_tail_s = float(support_payload.get("source_post_tail_s", np.nan))
            selected_source_command_active_start_s = float(support_payload.get("source_command_active_start_s", np.nan))
            selected_source_command_active_end_s = float(support_payload.get("source_command_active_end_s", np.nan))
            ...
            selected_support_reference_alignment_status = str(support_payload.get("support_reference_alignment_status", "active_window_missing"))
        ...
        interpolated_reference = np.asarray(support_payload.get("support_reference_field_mT", interpolated_field), dtype=float)
        ...
        reference_finite = np.isfinite(interpolated_reference)
        blended_reference_sum[reference_finite] += weight * interpolated_reference[reference_finite]
        blended_reference_weight[reference_finite] += weight
        support_rows.append(
            {
    ...
    blended_reference = np.full_like(time_grid, np.nan, dtype=float)
    reference_mask = blended_reference_weight > 0
    blended_reference[reference_mask] = blended_reference_sum[reference_mask] / blended_reference_weight[reference_mask]
    ...
    modeled = _build_finite_modeled_profile(
        support_payload={
            ...
            "support_reference_field_mT": blended_reference,
                "active_window_start_s": selected_active_window_start_s,
                "active_window_end_s": selected_active_window_end_s,
                "active_duration_s": float(active_duration_s),
                "source_total_duration_s": selected_source_total_duration_s,
                "source_pre_baseline_s": selected_source_pre_baseline_s,
                "source_post_tail_s": selected_source_post_tail_s,
                "source_command_active_start_s": selected_source_command_active_start_s,
                "source_command_active_end_s": selected_source_command_active_end_s,
    ...
    support_family_override_reason = None
    if support_family_override_applied:
        support_family_override_reason = (
            "requested_family_unavailable"
            if user_requested_support_family not in candidate_waveform_types
            else "cross_family_candidate_scored_better"
        )
    support_family_score_summary = (
        f"mode={support_family_selection_mode};requested={user_requested_support_family};"
        f"selected={selected_support_waveform or 'unknown'};"
        f"candidates={','.join(candidate_waveform_types) if candidate_waveform_types else 'none'}"
    )
    ...
    return {
        ...
        "requested_support_family": user_requested_support_family,
        ...
        **selected_support_source_contract,
        **cycle_selection_contract,
        "active_window_start_s": selected_active_window_start_s,
        "active_window_end_s": selected_active_window_end_s,
        "active_duration_s": float(active_duration_s),
        "zero_padded_fraction": float(selected_zero_padded_fraction),
...
def _support_source_file(entry: dict[str, Any]) -> str | None:
    for key in ("source_file", "source_original_path", "path", "file_path", "filename", "test_id"):
        value = entry.get(key)
...
def _support_cycle_selection_contract(
    *,
    requested_cycle_count: float | None,
    selected_support_cycle_count: float | None,
    support_count_used: int,
    exact_cycle_support_used: bool,
    tolerance: float = 0.05,
) -> dict[str, Any]:
    requested = float(requested_cycle_count) if requested_cycle_count is not None and np.isfinite(float(requested_cycle_count)) else float("nan")
    selected = (
        float(selected_support_cycle_count)
        if selected_support_cycle_count is not None and np.isfinite(float(selected_support_cycle_count))
        else float("nan")
    )
    if int(support_count_used) > 1:
        match_type = "weighted_blend"
    elif bool(exact_cycle_support_used):
        match_type = "exact"
    elif np.isfinite(requested) and np.isfinite(selected):
        match_type = "nearest"
    else:
        match_type = "fallback"
    override = bool(np.isfinite(requested) and np.isfinite(selected) and abs(requested - selected) > float(tolerance))
    reason = None
    if override:
        if match_type == "weighted_blend":
            reason = "weighted_support_blend_uses_representative_cycle_candidates"
        elif match_type == "nearest":
            reason = "nearest_cycle_candidate_selected"
    ...
    return {
        ...
        "support_cycle_override_reason": reason,
    }
...
def _nonzero_window(
    ...
    *,
    threshold_pp_fraction: float = 0.01,
) -> tuple[float, float]:
    ...
    pp = float(np.nanmax(finite_signal) - np.nanmin(finite_signal)) if finite_signal.size else float("nan")
    threshold = max(abs(pp) * float(threshold_pp_fraction), 1e-6) if np.isfinite(pp) else 1e-6
    nonzero = finite & (np.abs(signal) > threshold)
    if not nonzero.any():
        return float("nan"), float("nan")
    nonzero_time = time[nonzero]
    return float(np.nanmin(nonzero_time)), float(np.nanmax(nonzero_time))


def _shape_corr_and_nrmse(left: np.ndarray, right: np.ndarray) -> tuple[float, float]:
    ...
    left_centered = left_finite - float(np.nanmean(left_finite))
    right_centered = right_finite - float(np.nanmean(right_finite))
    denominator = float(np.linalg.norm(left_centered) * np.linalg.norm(right_centered))
    corr = float(np.dot(left_centered, right_centered) / denominator) if denominator > 1e-12 else float("nan")
    rmse = float(np.sqrt(np.nanmean(np.square(right_finite - left_finite))))
    pp = float(np.nanmax(left_finite) - np.nanmin(left_finite))
...
def _build_command_prediction_consistency_contract(command_profile: pd.DataFrame) -> dict[str, Any]:
    ...
    if (
        ...
        and np.allclose(
            ...
            pd.to_numeric(command_profile["compensated_recommended_voltage_v"], errors="coerce").to_numpy(dtype=float),
            equal_nan=True,
        )
    ):
    ...
    command_values = (
        pd.to_numeric(command_profile[plotted_command_column], errors="coerce").to_numpy(dtype=float)
        if plotted_command_column in command_profile.columns
        else np.full(len(command_profile), np.nan)
    )
    target_values = (
        pd.to_numeric(command_profile[target_column], errors="coerce").to_numpy(dtype=float)
        if target_column in command_profile.columns
        else np.full(len(command_profile), np.nan)
    )
    predicted_values = (
        pd.to_numeric(command_profile[predicted_column], errors="coerce").to_numpy(dtype=float)
        if predicted_column in command_profile.columns
        else np.full(len(command_profile), np.nan)
    )
    support_values = (
        pd.to_numeric(command_profile["support_reference_output_mT"], errors="coerce").to_numpy(dtype=float)
        if "support_reference_output_mT" in command_profile.columns
        else np.full(len(command_profile), np.nan)
    )
    command_start, command_end = _nonzero_window(time_values, command_values)
    target_start, target_end = _nonzero_window(time_values, target_values)
    sample_dt = float(np.nanmedian(np.diff(time_values[np.isfinite(time_values)]))) if np.isfinite(time_values).sum() > 1 else 1e-6
    tolerance_s = max(abs(sample_dt) * 2.0, 1e-6)
    covers_start = bool(
        np.isfinite(command_start)
        and np.isfinite(target_start)
        and command_start <= target_start + tolerance_s
    )
    covers_end = bool(
        np.isfinite(command_end)
        and np.isfinite(target_end)
        and command_end >= target_end - tolerance_s
    )
    corr, nrmse = _shape_corr_and_nrmse(target_values, support_values)
    support_shape_mismatch = bool(
    ...
    support_contamination = bool(
        ...
        and np.allclose(support_values, predicted_values, equal_nan=True)
    )
    predicted_from_command = bool(prediction_available and covers_start and covers_end and not support_contamination)
    status_parts: list[str] = []
    if not prediction_available:
        status_parts.append("forward_prediction_unavailable")
    if not covers_start or not covers_end:
        status_parts.append("command_coverage_insufficient")
    if support_contamination:
        status_parts.append("support_reference_contamination")
    if prediction_available and not predicted_from_command:
