...
def render_finite_first_phase_sync_review(command_profile: pd.DataFrame, metadata: dict[str, object]) -> None:
    ...
    summary = {
        ...
        "실측 field half-range (mT)": metadata.get("measured_abs_peak_effective_mT"),
        "±50mT 정규화 scale": metadata.get("measured_field_scale_to_50mT"),
        "정규화 center (mT)": metadata.get("measured_field_normalization_center_mT"),
        "전체 제거 offset (mT)": metadata.get("measured_field_total_offset_removed_mT"),
...
def _render_phase_sync_correction_basis(metadata: dict[str, object]) -> None:
    ...
    rows = [
        ...
        {"항목": "실측 min/max", "계산/의미": "smoothing 후 active 구간 min/max", "현재값": f"{metadata.get('measured_field_smoothed_active_min_mT')} / {metadata.get('measured_field_smoothed_active_max_mT')}"},
        {"항목": "정규화 center", "계산/의미": "(smoothed_active_max + smoothed_active_min) / 2", "현재값": metadata.get("measured_field_normalization_center_mT")},
        {"항목": "전체 제거 offset", "계산/의미": "pre-normalization baseline + smoothed midrange center", "현재값": metadata.get("measured_field_total_offset_removed_mT")},
        {"항목": "실측 half-range", "계산/의미": "(smoothed_active_max - smoothed_active_min) / 2", "현재값": metadata.get("measured_field_smoothed_half_range_mT")},
        {"항목": "±50mT scale", "계산/의미": "50 / measured_field_smoothed_half_range_mT", "현재값": metadata.get("measured_field_scale_to_50mT")},
